...
//checked={savedSettings[key]}

    return (<>
        <HeaderButton onClick={toggleDropdown} title={<>Settings
            {showDropdown && (
                <div ref={dropdownRef} className="absolute shadow-lg px-1.5 py-2.5 rounded-md bg-white border z-10 flex flex-col gap-1 min-w-24">
                    {Object.keys(savedSettings).map((key, index) => (
                        <label key={index}>
                            <input type="checkbox" onChange={(event) => handleOnChange(event, key)}  checked={savedSettings[key]}/>
                            {key}
                        </label>
                    ))}
                </div>
